...
import { ObjectiveRail } from './components/game/ObjectiveRail';
import { GrowthRoadmap } from './components/game/GrowthRoadmap';
...
} from './features/objectives/index.js';
function readOnboardingState() {
    try {
        const saved = localStorage.getItem('zenOnboardingState');
        return saved ? restoreOnboardingState(JSON.parse(saved)) : createInitialOnboardingState();
    } catch {
...
export default function App() {
    ...
    const objectives = useMemo(() => deriveObjectives(game), [game]);
    ...
    useEffect(() => {
        localStorage.setItem('zenOnboardingState', JSON.stringify(onboarding));
    }, [onboarding]);
    useEffect(() => {
        const shouldShowFirstRun = currentUser
            && !isLoading
            && !onboarding.active
            && !onboarding.completed
            && !onboarding.dismissed
            && Number(game.level) <= 1
            && Number(game.interactions) <= 0;
        if (shouldShowFirstRun) dispatchOnboarding({ type: 'start' });
    }, [currentUser, game.interactions, game.level, isLoading, onboarding.active, onboarding.completed, onboarding.dismissed]);
    ...
    const handleOpenModal = useCallback((name, opener) => {
        audio.playClick();
    ...
    const handleObjectiveNavigate = useCallback((objective) => {
        audio.playClick();
        if (objective.navigationTarget === 'event') {
            document.querySelector('.event-interaction')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return;
        }
        if (['store', 'companions', 'prestige'].includes(objective.navigationTarget)) {
            openModal(objective.navigationTarget);
        }
    }, [openModal]);
    ...
    const modalProps = {
        visibility, focusSuspended: onboarding.active, returnFocusRef: modals.returnFocusRef,
        game,
        currentUser,
        t,
        leaderboardData,
    ...
    return (
        <div className={`game-shell game-typography fixed inset-0 flex flex-col items-center transition-colors duration-1000 ${isDay ? 'bg-gradient-to-b from-blue-200 to-blue-100' : 'bg-gradient-to-b from-indigo-900 to-slate-800 text-white'} overflow-hidden`}>
            ...
            <div className="game-main-panel w-full flex-1 relative z-10 pb-6 pt-16 px-4">
                <aside className="game-support-rail game-support-rail-left">
                    <ObjectiveRail objectives={objectives} t={t} onNavigate={handleObjectiveNavigate} />
                </aside>
                <div className="game-scene-column">
                    <GameStage
                ...
                <aside className="game-support-rail game-support-rail-right">
                    <GrowthRoadmap level={game.level} season={game.season} t={t} />
                </aside>
            </div>
            <ActionPanel
                game={game}
            ...
            <OnboardingOverlay
                state={onboarding}
                t={t}
                onBack={() => dispatchOnboarding({ type: 'back' })}
                onNext={() => dispatchOnboarding({ type: 'next' })}
                onDismiss={() => dispatchOnboarding({ type: 'dismiss' })}
                onComplete={() => dispatchOnboarding({ type: 'complete' })}
            />
            <div className="absolute bottom-2 w-full px-4 pointer-events-none flex flex-col-reverse items-center gap-1 z-30 h-12 justify-end" role="status" aria-live="polite" aria-atomic="true">
